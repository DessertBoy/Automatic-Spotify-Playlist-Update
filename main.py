...
"""
username: Spotify username, must be a string
password: Spotify password, must be a string
playlist_id: The ID of our playlist, must be a string
playlist_url: The endpoint that we need to send our post request to in order to update our playlist with our new songs. Must be a string.
...
def token_refresher():
	"""
	Every Spotify API endpoint requires a token to be used/access. This function will automatically retrieve a new token for us in order to be able to udpate our playlist.
	Before retrieving a token you need to specify your scopes.
...
def retrieve_songid():
	...
	This function will help us retrieve all the song ID's that are currently in our playlist. Once we have those ID's we will be able to remove the songs from our playlist
...
def song_removal(song_ids):
	...
	#Deleting songs from our playlist
...
def album_search():
	...
	For this function it is highly recommended you understand basic HTML.
...
def retrieve_album_uri(discography):
	...
	#Checking if the albums we scraped are available on Spotify
...
def retrieve_songs(album_ids):
	...
	#Choosing a random song from each album that was scraped and is available on Spotify
	...
	#Updating our playlist with the newly selected random songs
